...
// Loader data for the settings page (ported from src/app/(app)/settings/page.tsx).
// A server fn so the service-role AiCredential read stays off the client bundle;
...
async function loadAiCredential(projectId: string): Promise<AiCredentialView | null> {
  ...
  const { data: cred } = await supabaseAdmin
    .from("AiCredential")
    .select(
      "provider, mode, model, apiKey, oauthAccessToken, oauthExpiresAt, deepseekApiKey, openaiApiKey, codexOauthAccessToken, creator:User!AiCredential_createdBy_fkey(email)",
    )
  ...
  return {
    ...
    hasOpenAiKey: !!cred.openaiApiKey,
    hasCodexSubscription: !!cred.codexOauthAccessToken,
    model: cred.model,
...
export const getSettingsData = createServerFn({ method: "POST" }).handler(
  async () => {
